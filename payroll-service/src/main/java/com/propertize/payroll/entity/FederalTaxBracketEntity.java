...
/**
 * Entity representing federal tax brackets for income tax calculation.
 */
@Entity
@Table(name = "federal_tax_brackets", indexes = {
    @Index(name = "idx_fed_bracket_year", columnList = "taxYear"),
    @Index(name = "idx_fed_bracket_status", columnList = "filingStatus"),
    @Index(name = "idx_fed_bracket_min", columnList = "minIncome")
})
@Getter
@Setter
public class FederalTaxBracketEntity extends BaseEntity {
    ...
    /**
     * Tax rate for this bracket (e.g., 0.22 for 22%)
     */
    @Column(nullable = false, precision = 5, scale = 4)
    private BigDecimal taxRate;

    /**
     * Base tax amount from lower brackets
     */
    @Column(precision = 15, scale = 2)
