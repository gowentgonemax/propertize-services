...
class DefaultPolicyEngineTest {
    ...
    @BeforeEach
    void setUp() {
        policyEngine = new DefaultPolicyEngine(rbacService, List.of(ownershipEvaluator));
    }
    ...
    class EvaluateTests {
        ...
        void testDecisionMetadata() {
            ...
            assertEquals("2.0.0", decision.getConfigVersion());
            assertEquals("policy-v2", decision.getPolicyId());
    ...
    @Test
    @DisplayName("Should return config version 2.0.0")
    void testGetConfigVersion() {
        assertEquals("2.0.0", policyEngine.getConfigVersion());
    }
}
